...
while True:
    ...
    if current_total_halite / NUMBER_OF_PLAYERS < 25000 and game_phase == 'gathering':
    ...
    if game_phase == 'gathering':
        ...
        if len(interesting_coords) > 800:
            min_halite_to_be_interesting *= 1.2
        elif len(interesting_coords) < 400:
            min_halite_to_be_interesting /= 1.2

    #ZONE_OF_INTEREST.append(temp_row)
    ...
    for ship in ship_In_Priority_Order:
        ...
        if ship_States[ship.id] == "collecting":
            if not halite_Dict:
                ...
                #logging.info(f"Following path since Ship {ship.id} ship_in_dead_zone is {ship_in_dead_zone[ship.id]}")
