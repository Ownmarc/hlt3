...
class GameMap:
    ...
    def smarter_naive_navigate(self, ship, destination, position_choices):
        """
        Returns a singular safe move towards the destination.
        ...
        for direction in self.get_unsafe_moves(ship.position, destination):
            target_pos = ship.position.directional_offset(direction)
            if target_pos not in position_choices:
                return direction
        return (0, 0)

    def smarter_naive_navigate_2(self, ship, destination, position_choices):
        """
        Returns a singular safe move towards the destination.
        ...
        for direction in self.get_unsafe_moves(ship.position, destination):
            target_pos = ship.position.directional_offset(direction)
            if target_pos not in position_choices:
                optimals_directions.append(direction)
        if len(optimals_directions) > 0:
            return optimals_directions
        else:
            return [(0, 0)]

    def suicide_in_base(self, ship, destination):
        for direction in self.get_unsafe_moves(ship.position, destination):
            return direction
